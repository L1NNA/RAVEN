...
def _build_str_matrix(training_files, field, cache_folder=None,
    ...
    vecs_flt = []
    for v in vecs.items():
        if len(v[1].shape) == 2 and v[1].shape[1] == len(features):
            vecs_flt.append(v[1])
        else:
            log_warn('found one invalid feature. {}', v[1].shape)
    mat = vs(vecs_flt)
...
class RavenPreprocessor:
            ...
            with tf.name_scope('embds_visualization'):
                ...
                if os.path.exists(tps_cache_file):
                    ...
                    with ProcessPoolExecutor() as e:
                        ...
                        for st, cnt in tqdm(ite, total=len(fss)):
                            str_tps.update(st[0])
                            imp_tps.update(st[1])
                ...
                str_cnts = sorted(str_cnts.items(), key=lambda _: (
                    _[1], _[0]), reverse=True)[:topk_tfboard//10]
                imp_cnts = sorted(imp_cnts.items(), key=lambda _: (
                    _[1], _[0]), reverse=True)[:topk_tfboard//10]

                var_str = vs([str_tps[w] for w, _ in str_cnts if len(str_tps[w].shape) == 2])
                var_str = self.__str_mod.transform(var_str)
                var_imp = vs([imp_tps[w] for w, _ in imp_cnts if len(imp_tps[w].shape) == 2])
    ...
    def model_obj(self, obj, as_tfr=False, pre_cal_asm_embd=False):
        ...
        if k.strip().lower() in ('.data', '.rdata', '.rsrc', '_data', '_rdata', '_rsrc')]
        if len(dat_vals) < 1:
            dat_vals = [0]
    ...
    @staticmethod
    def get_attn(obj, blk_ord, probs, ind1, stride=500, topk=10):
        # men_len = len(blk_ord) + len(obj['str']) + len(obj['imp_f']) + len(obj['data'])
        # if len(probs) != men_len:
        #     log_warn("Unmatched attention dim for {} - {} vs {} ",
        #              obj['sha256'], len(probs), men_len)
        idx = np.argsort(-probs, axis=-1)[..., :topk]
        weights = probs[idx].tolist()
        eles = []
        len_asm = len(blk_ord)
        len_str = len(obj['str']) if 'str' in obj else 0
        len_imp = len(obj['imp_f']) if 'imp_f' in obj else 0
        dat_vals = [bt for k in obj['data'] for bt in obj['data'][k] 
        if k.strip().lower() in ('.data', '.rdata', '.rsrc', '_data', '_rdata', '_rsrc')]
        for ele_id in idx:
            ele_id = ind1[ele_id] # pick from pre-selection
            if ele_id < len(blk_ord):
                eles.append(obj['asm'][blk_ord[ele_id]])
            elif ele_id < len(blk_ord) + len_str:
                ele = obj['str'][ele_id - len(blk_ord)]
                if isinstance(ele, tuple):
                    ele = ele[0]
                eles.append(ele)
            elif ele_id < len(blk_ord) + len_str + len_imp:
                ele = obj['imp_f'][ele_id - len(blk_ord) - len_str]
                if isinstance(ele, tuple):
                    ele = ele[0]
                eles.append(ele)
            else:
                offset = ele_id - len(blk_ord) - len_str - len_imp
                offset *= stride
                eles.append(dat_vals[offset:offset+500])
        return eles, weights
...
def build_pps(ps_folder, training_files, cache_folder, kam1n0exp_jar, protocol,
    ...
    if ps is None:
        from md_raven_ds import RavenPreprocessor
...
def build_training_data_and_ps(
        ...
        rsk_min_freq=20, ds_suffix='', external_data='', 
        **kwargs):
    ...
    if len(external_data) > 0:
        ext = [os.path.join(external_data, p) for p in os.listdir(external_data)]
        file_ext = os.path.join(tfr_folder, 'ext')
        dps.write_files_as_tfr_trnks(ext, file_ext, ps.model_obj)

    log('done.')
    return load_training_data(ds_folder, ds_suffix=ds_suffix)


def load_training_data(ds_folder, map_parallelism=8, ds_suffix='', **kwargs):
...
def set_flags(oparser):
    ...
    parser.add_argument(
        ...
        default="",
        metavar="",
        help="External data for evaluation.")
    parser.add_argument(
        "--ds_folder",
        type=str,
        default="data/small",
